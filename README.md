...
2. Select repo **Engineer00/AI-Invoice-2026**
3. Under **Settings** → **Source**:
   - **Root Directory**: `AI Invoice Intelligence Dashboard`
4. Under **Variables**, add:
   - `VITE_API_BASE_URL` = `https://ai-invoice-2026-production.up.railway.app` *(your backend URL, NOT the frontend; no trailing slash)*
5. **Settings** → **Networking** → **Generate Domain**

The frontend Dockerfile builds the React app and serves it. Each service gets its own URL.
...
1. Create a new project on [Railway](https://railway.app) → **New Service from GitHub**
2. Select this repo. **Leave Root Directory empty** (repo root required for `invoice_extractor` import)
3. Add environment variable: `GEMINI_API_KEY` (copy from your `invoice_extractor/.env`)
4. Generate a public domain in **Settings** → **Networking** → **Generate Domain**

Config: `railway.toml` and `nixpacks.toml` at repo root handle build and start automatically.
