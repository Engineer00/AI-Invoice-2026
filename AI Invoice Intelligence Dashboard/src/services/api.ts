...
export function getDocumentFileUrl(documentId: string, pageNo?: number): string {
  const hash = pageNo != null ? `#page=${pageNo}` : '';
  return `${API_BASE}/api/documents/${documentId}/file${hash}`;
}

/** Resolve API-returned relative URL (e.g. document_url) to absolute when needed */
...
export async function listJobs(limit = 50) {
  const res = await fetch(`${API_BASE}/api/jobs?limit=${encodeURIComponent(String(limit))}`);
  if (!res.ok) throw new Error(await res.text());
  return (await res.json()) as ApiJobStatus[];
}
...
export async function exportInvoicesXlsx(): Promise<{ blob: Blob; filename: string }> {
  const res = await fetch(`${API_BASE}/api/invoices/export.xlsx`);
  if (!res.ok) throw new Error(await res.text());
...
export async function listParties(party_type?: 'supplier' | 'buyer') {
  ...
  const res = await fetch(`${API_BASE}/api/parties${qs}`);
  if (!res.ok) throw new Error(await res.text());
...
export async function requestRescan(id: string, payload: { reasons?: string[]; unreadable_fields?: string[] }) {
  const res = await fetch(`${API_BASE}/api/invoices/${id}/request-rescan`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
...
export async function reuploadInvoice(id: string, file: File) {
  const form = new FormData();
  form.append('file', file);
  const res = await fetch(`${API_BASE}/api/invoices/${id}/reupload`, { method: 'POST', body: form });
  if (!res.ok) throw new Error(await res.text());
  return (await res.json()) as ApiInvoiceDetail;
}
